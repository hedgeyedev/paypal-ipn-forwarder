Feature: forward an IPN from PayPal to a development machine
  ...
  So that I can more fully simulate the PayPal production environment in my development space.

  Scenario: PayPal sandbox sends IPN to common server
    Given that the sandbox has a recurring payment to notify Hedgeye about
  ...
  Scenario: Server sends IPN to specific developer computer
    Given that the server has an IPN in the queue
  ...
  Scenario: Developer computer is offline and the server bangs on it until it comes back
    Given that the server has an IPN to send to the developer computer
